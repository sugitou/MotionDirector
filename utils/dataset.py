...
def time_resample_linear(video: torch.Tensor, target_frames: int) -> torch.Tensor:
    """
    video: [N, C, H, W] (uint8/float both are acceptable)
    target_frames: number of output frames T
    ...
    u = torch.linspace(0.0, float(N - 1), steps=T, device=dev)  # Evenly divide [0, N-1]
...
# Not used due to decreased visual quality
...
def time_resample_flow(video: torch.Tensor, target_frames: int,
    ...
    """
    video: [N, C, H, W] (uint8/float) assumed 0..255
    target_frames: number of output frames T
    algo: "farneback" (lightweight, no dependencies)
    symmetric_blend:
        True  -> Warp Fi forward by a, warp Fj backward by (1-a), blend temporally
        False -> Warp Fi forward by a only and blend linearly (one side only; faster)

    Returns: [T, C, H, W] (float32, 0..255)
    """
    assert video.ndim == 4
    N, C, H, W = video.shape
    T = int(target_frames)
    ...
    # Tensor -> numpy (HxWxC, BGR is not required. cv2.remap does not care about channel order)
    vid_np = video.detach().cpu().to(torch.uint8).permute(0, 2, 3, 1).numpy()  # [N, H, W, C]
    # Optical flow can be calculated in grayscale
    gray = [cv2.cvtColor(f, cv2.COLOR_RGB2GRAY) if vid_np.shape[3] == 3 else f[...,0] for f in vid_np]

    # Precompute flow for each pair (i->i+1 and (i+1)->i)
    flows_fwd = []  # [N-1, H, W, 2]
    flows_bwd = []  # [N-1, H, W, 2]
    # Recommended Farneback settings (fast enough for ~256x192)
    ...
    for t in range(T):
        u = (N - 1) * t / max(1, T - 1)      # Continuous time u ∈ [0, N-1]
        i = int(np.floor(u))
        if i >= N - 1:
        ...
        # Get flow
        fwd = flows_fwd[i]                   # [H,W,2] (x,y order)
        if symmetric_blend:
            bwd = flows_bwd[i]
        else:
            bwd = None

        # Scale forward/backward warp amount
        ...
        # remap (replicate at boundaries is OK)
        ...
        if symmetric_blend:
            warp_j = cv2.remap(Fj, map_x_j, map_y_j, interpolation=cv2.INTER_LINEAR,
                               borderMode=cv2.BORDER_REPLICATE)
            # Blend temporally according to a (transition from Fi to Fj)
            out[t] = (1.0 - a) * warp_i + a * warp_j
        else:
            # One side only: move Fi to the future, blend linearly with Fj
...
class SingleVideoDataset(Dataset):
    ...
    def create_video_chunks(self):
        ...
        vr_range = list(range(0, len(vr), self.frame_step))  # Valid index list

        self.frames = []
        size = self.n_sample_frames
        # Keep the last chunk even if it's smaller than size (no padding here)
        for i in range(0, len(vr_range), size):
            part = vr_range[i:i+size]          # The last part may be less than size
            if len(part) > 0:
                self.frames.append(tuple(part))
        return self.frames
    
    def get_frame_batch(self, vr, resize=None):
        # Actual frame index list to retrieve
        ...
        # For stable image quality: spatial resize → temporal interpolation is recommended
        if resize is not None:
            video = resize(video)

        # Always perform temporal resampling (interp) to n_sample_frames here
        if video.shape[0] != self.n_sample_frames:
            video = time_resample_linear(video, self.n_sample_frames)
            #video = time_resample_flow(video, self.n_sample_frames, symmetric_blend=True)
